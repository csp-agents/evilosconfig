...
var (
	...
	watchConfigRetryInterval   = 1 * time.Second
...
func getMetadata(suffix string) ([]byte, string, error) {
	...
	if net.ParseIP(h) == nil && h != "metadata" && h != "metadata.google.internal" {
		scheme = "https"
	}
	computeMetadataURL := scheme + "://" + host + "/computeMetadata/v1/" + suffix
...
func WatchConfig(ctx context.Context) error {
	...
	for {
		...
		// Try up to 3 times (~6s) to wait for slow network initialization, after
